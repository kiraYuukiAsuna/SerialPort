...
namespace SerialPortUtils {
	 ...
	 * 5. Get the friendly name based on looking up the collected registry
	 * values.
	 *
	...
	std::vector<SerialPortInfo> SerialPort::getSerialPortList() {
		...
		while (!stopTosearch) {
			...
			// Collect device name
			const DWORD collectDevicesStatus =
				QueryDosDevice(nullptr, &(devices[0]), deviceBufferSize);
		...
		while (devices[deviceStartCharLoc] != _T('\0')) {
			...
			if (currentDeviceNameLen > 3 &&
				_tcsnicmp(currentDevice, _T("COM"), 3) == 0) {
				...
				if (!portNumber.empty()) {
					...
					// Get Device name
					currentSerialPortInfo.queryDosDeviceName =
						new TCHAR[currentDeviceNameLen];
#pragma warning(suppress : 4996)
					std::_tcscpy(currentSerialPortInfo.queryDosDeviceName,
								 currentDevice);

					// Get serial port location
					LPTSTR currentDeviceName = new TCHAR[300];
					const DWORD charCount =
						QueryDosDevice(currentDevice, currentDeviceName, 300);

#ifdef _UNICODE
					std::wstring currentDeviceNameWStr(currentDeviceName);
					currentSerialPortInfo.deviceName =
						std::string(currentDeviceNameWStr.begin(),
									currentDeviceNameWStr.end());
#else
		...
		if (!result.empty()) {
			...
			processRegistryValue(
				...
				[&friendNames](std::string valueName, DWORD dataType,
							   unsigned char* data, int dataLen) {
					if (valueName.compare("FriendlyName") == 0 &&
						dataType == REG_SZ) {
...
#pragma region Connection
	 ...
	 */
	bool SerialPort::open(SerialPortInfo port) { return open(port.port); }
	 ...
	 * @brief Open Serial Port
	 *
	 * @param[in] port The Port name to be openned. If you want to open COM3,
	 * port = "COM3" or "\\\\.\\COM3"
	 * @return Return true if connection success, otherwise return false
	 */
	bool SerialPort::open(std::string port) {
		...
		if (!m_connected) {
...
#endif
			m_serialHandle =
				CreateFile(portLPCSTR, GENERIC_READ | GENERIC_WRITE,
			...
			if (m_serialHandle != INVALID_HANDLE_VALUE) {
				...
				bool setBuffer = SetupComm(
					m_serialHandle, m_rxtxBufferSize,
					m_rxtxBufferSize);	// Set the input and output buffer
				PurgeComm(m_serialHandle,
						  PURGE_TXCLEAR | PURGE_RXCLEAR);  // Reset buffer
...
#pragma region Setting
	...
	/**
	 * @brief  A subfunction to set flow contorl parameter. It must be used with
...
#pragma region Getter and Setter
	 ...
	 * @brief Set baudrate
	 *
	 * @param[in] baudRate Baudrate: 110, 300, 600, 1200, 2400, 4800, 9600,
	...
	bool SerialPort::setBaudRate(int baudRate) {
		if (baudRate != CBR_110 && baudRate != CBR_300 && baudRate != CBR_600 &&
			baudRate != CBR_1200 && baudRate != CBR_2400 &&
			baudRate != CBR_4800 && baudRate != CBR_9600 &&
			baudRate != CBR_14400 && baudRate != CBR_19200 &&
			baudRate != CBR_38400 && baudRate != CBR_57600 &&
			baudRate != CBR_115200 && baudRate != CBR_128000 &&
			baudRate != CBR_256000)
			throw std::invalid_argument("baudRate(" + std::to_string(baudRate) +
										") is invalid.");
	 ...
	 */
	int SerialPort::getBaudRate() { return m_baudRate; }

	/**
	 * @brief Set byte size
	 *
	 * @param byteSize The number of bits in the bytes transmitted and received.
	 * (4-8)
	 * @return Return true if success.
	 */
	bool SerialPort::setByteSize(int byteSize) {
		...
		if (byteSize < 4 || byteSize > 8)
			throw std::invalid_argument("byteSize(" + std::to_string(byteSize) +
										") must be 4 - 8.");
	 ...
	 * @return int
	 */
	int SerialPort::getByteSize() { return m_byteSize; }
	...
	bool SerialPort::setStopBits(int stopBits) {
		// Check
		if (stopBits != ONESTOPBIT && stopBits != ONE5STOPBITS &&
			stopBits != TWOSTOPBITS)
			throw std::invalid_argument("stopBits(" + std::to_string(stopBits) +
										") must be 1, 1.5 or 2.");
	 ...
	 */
	int SerialPort::getStopBits() { return m_stopBits; }

	/**
	 * @brief Set parity
	 *
	 * @param[in] parity It must be NOPARITY, ODDPARITY, EVENPARITY, MARKPARITY
	 * or SPACEPARITY
	 * @return Return true if success
	 */
	bool SerialPort::setParity(int parity) {
		// Check
		if (parity != NOPARITY && parity != ODDPARITY && parity != EVENPARITY &&
			parity != MARKPARITY && parity != SPACEPARITY)
			throw std::invalid_argument(
				"stopBits(" + std::to_string(parity) +
		...
		if (m_connected) {
			...
			result = setSerialStateDecorator(
				[parity](DCB& serialParams) { serialParams.Parity = parity; });
	 ...
	 * @brief Get Parity
	 *
	 * @return Return parity as NOPARITY, ODDPARITY, EVENPARITY, MARKPARITY or
	 * SPACEPARITY
	 */
	int SerialPort::getParity() { return m_parity; }

	/**
	 * @brief Set flow contorl
	 *
	 * @param flowControl SERIAL_PORT_FCTL_NONE, SERIAL_PORT_FCTL_XON_XOFF or
	...
	bool SerialPort::setFlowControl(int flowControl) {
		// Check
		if (flowControl != SERIAL_PORT_FCTL_NONE &&
			flowControl != SERIAL_PORT_FCTL_XON_XOFF &&
			flowControl != SERIAL_PORT_FCTL_HARDWARE)
			throw std::invalid_argument(
				"flowControl(" + std::to_string(flowControl) +
				") must be SERIAL_PORT_FCTL_NONE, SERIAL_PORT_FCTL_XON_XOFF or "
				"SERIAL_PORT_FCTL_HARDWARE.");
		...
		if (m_connected) {
			...
			// Set
			result =
				setSerialStateDecorator([this, flowControl](DCB& serialParams) {
	 ...
	 *
	 * @return SERIAL_PORT_FCTL_NONE, SERIAL_PORT_FCTL_XON_XOFF or
	 * SERIAL_PORT_FCTL_HARDWARE
	 */
	int SerialPort::getFlowControl() { return m_flowControl; }
	...
	bool SerialPort::setEndOfChar(char endOfChar) {
		...
		if (m_connected) {
			...
			// Set
			result = setSerialStateDecorator([endOfChar](DCB& serialParams) {
	 ...
	 * @return char
	 */
	char SerialPort::getEndOfChar() { return m_endOfChar; }
	 ...
	 * @return int
	 */
	int SerialPort::getTimeout() { return m_timeout; }

	/**
	 * @brief Set up the Rxtx hardware buffer size. Default is 1024. If the
	 * protocol packets is larger, please make it bigger.
	 *
	...
	bool SerialPort::setRxTxBufferSize(int bufferSize) {
		...
		if (m_connected) {
			...
			// Set
			result = SetupComm(m_serialHandle, bufferSize,
							   bufferSize);	 // Set the input and output buffer
	 ...
	 * @return
	 */
	int SerialPort::getRxTxBufferSize() { return m_rxtxBufferSize; }
	 ...
	 */
	bool SerialPort::isOpened() { return m_connected; }
...
#pragma region Transmission
	...
	int SerialPort::sendBytes(unsigned char* buffer, int bufferSize) {
		...
		if (m_connected) {
			int n;
			if (WriteFile(m_serialHandle, buffer, bufferSize,
						  (LPDWORD)((void*)&n), NULL))
				result = n;
			else
				result = 0;
	 ...
	 */
	std::string SerialPort::readASCII() { return readASCII(1024); }
	...
	std::string SerialPort::readASCII(int strBufferSize) {
		// Read
		std::unique_ptr<unsigned char[]> buffer =
			std::unique_ptr<unsigned char[]>(new unsigned char[strBufferSize]);
	...
	int SerialPort::readBytes(unsigned char* buffer, int bufferSize) {
		...
		if (m_connected) {
			...
			// Read
			int n;
			if (ReadFile(m_serialHandle, buffer, bufferSize - 1,
						 (LPDWORD)((void*)&n),
						 NULL))	 // The nNumberOfBytesToRead is bufferSize-1
								 // instead of bufferSize because a '\0' may be
								 // added at Ipbuffer[n-1]
			{
				// Add end of string
				buffer[n] = '\0';
				result = n;
