...
namespace SerialPortUtils {
	/**
	 * @brief A looper for serial port. You can set any lambda function to
	 * handle the send or read process.
	 *
	 ...
	 *                Prepare your byte to be sent. Return (Bytes, lenght of
	 * bytes) end note -d-> "Send to serial" #b7285c -d-> "Send Post-Process"
	 *            note right
	 *                **setSendPostProcess()**
	 ...
	 *            note right
	 *                Enable this function by setting clearBUfferAfterSent() as
	 * true if unsigned char* should be released after sent. end note
	 *        }
	...
	class SerialPortLooper {
	   public:
		 ...
		 */
		typedef std::function<std::pair<unsigned char*, int>()>
			SendBytePreProcess;
		 ...
		 */
		typedef std::function<void(unsigned char* buffer, int bufferSize)>
			ReadByteProcess;
		 ...
		 *
		 */
		typedef std::function<void(std::vector<std::string> buffer)>
			ReadStringLineProcess;

		/**
		 * @ref setStartProcess(), setStopProcess(), setReadPreProcess()
		 */
		typedef std::function<void(std::unique_ptr<SerialPort>& serialPort)>
			SerialProcess;
